...
public class S3Service {
        ...
        public String uploadFile(MultipartFile file, String folder) throws IOException {
                ...
                // 1. Làm sạch tên file gốc (Xóa ký tự đặc biệt, khoảng trắng tránh lỗi URL)
                ...
                // 2. Tạo khóa định danh duy nhất (Key) trên S3 kèm cấu trúc folder
                // Định dạng: folder/UUID-filename.jpg
                ...
                try {
                        // 3. Xây dựng request cấu hình Object
                        PutObjectRequest putObjectRequest = PutObjectRequest.builder()
                                        .bucket(bucketName)
                                        .key(key)
                                        .contentType(file.getContentType()) // Giữ nguyên định dạng file (image/jpeg,
                                                                            // v.v.)
                                        .build();

                        // 4. Đẩy dữ liệu byte của file lên S3
                        s3Client.putObject(
                                        putObjectRequest,
                                        RequestBody.fromBytes(file.getBytes()));

                        // 5. Trả về đúng định dạng Public URL chuẩn hóa toàn cầu của AWS S3
